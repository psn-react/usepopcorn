...
export default function StarRating({ maxRating = 3, defaultRating = 0, color = "#fcc419", size = 48, messages = [], userSetRating }) {
  ...
  return (
    <div style={containerStyle}>
      ...
      <p style={textStyle}>
        {messages.length === maxRating ? messages[rating - 1] : tempRating || rating || null}
...
function Star({ clickOnRate, isFull, hoverIn, hoverOut, color, size }) {
  const starStyle = {
    width: `${size}px`,
    height: `${size}px`,
    display: "block",
  ...
  return (
    <span role="button" style={starStyle} onClick={clickOnRate} onMouseEnter={hoverIn} onMouseLeave={hoverOut}>
      {isFull ? (
        <svg
